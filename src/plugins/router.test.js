...
describe('router plugin', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    // create a stub server with route & register and logger
  ...
  it('warns and registers test endpoints when flag is true', async () => {
    ...
    // supply a dummy import so the router code sees it when it imports
    // we just need to ensure server.register called with the imported object
    // since testEndpoints is imported inside module we can't override easily,
    // but we can temporarily stub the import by mocking the module path
    const mod = await import('./router.js')
    // call register from fresh module instance
    await mod.router.plugin.register(server)
